function [Y1,U_tilde] = test_TTN_integrator_complex_parallel_nonglobal(tau,Y0,F_tau,t0,t1,A,d,r_min)
...
for i=1:m
    ...
    if 0 == iscell(tau{i})    % if \tau_i = l, l \in L
        ...
        [Y1_i,U_tilde{i}] = test_TTN_integrator_complex_parallel_nonglobal(tau{i},Y0_i,F_tau_i,t0,t1,A,d,r_min);
    end
    ...
    if 1 == iscell(Y1_i)
        ...
        % compute ONB of range of [core1 core2]
%         [W_taui_hat,~] = qr([core1 core2],0);
%         [~,s] = size(core1);
...
%         W_taui_hat(:,1:s) = core1; % don't need that anymore as we use Gram Schmidt
        ...
        U_tilde{i}{end} = tensor(U_tilde{i}{end},sz); % SIZE OF U_TILDE MUST BE ADJUSTED OVER LEVELS
...
%         sz = size(Y1_i{end});
%         U_tilde{i}{end} = Y1_i{end};  % CHECK DEFINITION OF U_TILDE AGAIN!!
%         U_tilde{i}{end-1} = eye(sz(end),sz(end));
        
%         sz = size(Y1_i{end});
%         sz2 = size(Y0{i}{end});
%         tmp = double(tenmat(Y1_i{end},m2+1,1:m2)).';
%         tmp = tmp(:,(sz2(end)+1):end);
%         sz_new = sz;
%         [~,sz_new(end)] = size(tmp);
%         U_tilde{i}{end} = mat2tens(tmp.',sz_new,m2+1);  % CHECK DEFINITION OF U_TILDE AGAIN!!
%         U_tilde{i}{end} = tensor(U_tilde{i}{end},sz_new);
%         U_tilde{i}{end-1} = eye(sz_new(end),sz_new(end));
        
    else
        [~,rr] = size(Y0{i});
        [Y1_i,~] = qr([Y0{i} Y1_i],0); 
%         Y1_i = mgson([Y0{i} Y1_i]); % using modified Gram Schmidt
...
%     U_tilde{end} = C1_bar;
    if aug(1,jj) ~= 0 
        ...
        % adjust U_tilde core tensor, such that the core is also augmented in 0-direction
%         if 1==iscell(U_tilde{jj})
...
%             s2 = size(C1_hat);
%             aug_sz = s1(end) - s2(jj);
%             mat_Ci = mat_Ci(:,1:aug_sz);
%             s_Ci = size(mat_Ci);
%             ss(jj) = ss(jj) + s_Ci(2);
%         else
%             s_Ci = size(Ci);
%             ss(jj) = ss(jj) + s_Ci(2);
%             [aug_sz,~] = size(mat_Ci); % ss(jj) - s_tmp(2);
%         end
%         s_Ci = size(Ci);
%         
%         s_Ci = prod(s_Ci(vv));
%         tmp(rr(jj)+1:rr(jj)+aug_sz,1:s_Ci) = mat_Ci;
%         C1_hat = tensor(mat2tens(tmp,ss,jj),ss);
    end
    % test - set core tensor of U_tilde 
